...
4. The system sends a one-time password setup link by email or another approved
   delivery channel.
5. The user opens the setup link and sets a permanent password.
...
The current preview uses an allowlist flow. With `DATABASE_URL`, admin/respondent
logins are read from the database auth tables where available. First-time
database users receive one-time setup links; plaintext temporary passwords are
not sent by the production onboarding helpers. Without a database, local
development falls back to the allowlist in `src/lib/demo-allowlist.ts`.
...
- Resend or another email provider for password setup links and daily respondent
  survey requests

Production deployments should set `UGA_INDEX_RUNTIME_MODE=production`, require
`DATABASE_URL`, keep setup links one-time/digest-only, audit account and role
changes, and enforce access control at both page and data-access layers.
