...
function HomeAiBriefBlock({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="bg-[#050505] px-6 pb-12 text-[#f8f8f2] lg:px-8">
      <div className="mx-auto grid max-w-[1900px] gap-5 rounded-[1.35rem] border border-white/12 bg-[#101010] p-5 lg:grid-cols-[0.8fr_auto] lg:items-center">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--spike-accent)]">
            {copy.eyebrow}
          </p>
          <h2 className="mt-3 text-2xl font-black uppercase leading-none tracking-normal text-white sm:text-3xl">
            {copy.title}
          </h2>
          <p className="mt-4 max-w-4xl text-sm leading-6 text-white/64">
            {copy.body}
          </p>
        ...
        <Link
          className="inline-flex justify-center rounded-full border border-white/22 bg-[#f8f8f2] px-5 py-3 text-sm font-black uppercase tracking-[0.12em] text-[#050505] transition hover:border-[var(--spike-accent)] hover:bg-[var(--spike-accent)]"
